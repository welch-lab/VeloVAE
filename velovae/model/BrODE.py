...
from .model_util import  hist_equal, convert_time, init_params, get_ts_global, reinit_type_params, pred_su, get_gene_index
    ...
    def __init__(self, 
        ...
        if(checkpoint is not None):
            ...
            if(param_key is not None):
                ...
                alpha, beta, gamma, scaling, ts, u0, s0, sigma_u, sigma_s, T, Rscore = init_params(X, p, fit_scaling=True)
            ...
            alpha, beta, gamma, u0, s0 = reinit_type_params(U/scaling, S, t, ts, cell_labels_int, cell_types_int, cell_types_int)
...
class BrODE():
    ...
    def update_std_noise(self, train_set):
        G = train_set.G
        Uhat, Shat, ll = self.pred_all(train_set.data, torch.tensor(train_set.time).double().to(self.device), train_set.labels, train_set.N, train_set.G, np.array(range(G)))
    ...
    def train(self, 
        ...
        for epoch in range(n_epochs):
            ...
            if(plot and (epoch==0 or (epoch+1) % self.config["save_epoch"] == 0)):
                ...
                print(f"Epoch {epoch+1}: Train Log Likelihood = {ll_train:.3f}, Test Log Likelihood = {ll:.3f}, \t Total Time = {convert_time(time.time()-start)}")
    ...
    def test(self,
        ...
        Uhat, Shat, ll = self.pred_all(dataset.data, torch.tensor(dataset.time).double().to(self.device), dataset.labels, dataset.N, dataset.G, gind)
