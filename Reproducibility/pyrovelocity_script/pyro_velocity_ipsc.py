...
import anndata
import time
ipsc_adata_raw = anndata.read_h5ad('/data/scRNA/IPSC/IPSC.h5ad')

ipsc_adata_processed = preprocess_dataset(data_set_name = 'IPSC',
                                          adata = ipsc_adata_raw,
                                          data_processed_path = '/data/external',
                                          n_top_genes = 2000)

#model1
start=time.time()
result = train_dataset(ipsc_adata_processed,
                       data_set_name = 'IPSC',
                       offset=False,
