...
import anndata
import time
braindev_adata_raw = anndata.read_h5ad('/data/scRNA/Braindev/Braindev_pp.h5ad')

braindev_adata_processed = preprocess_dataset(data_set_name = 'braindev',
                                              adata = braindev_adata_raw,
                                              data_processed_path = '/data/external',
                                              n_top_genes = 1000)

#model1
start=time.time()
result = train_dataset(braindev_adata_processed,
                       data_set_name = 'braindev',
                       offset=False,
